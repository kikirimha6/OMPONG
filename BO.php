...
$data = '{"/go-promotions/v1/promotions/enrolment", $token, $data);{"promo_code":"PAKEGOFOOD0906"}';
$claim = request($boba09,'"message":"','"');
message = fetch_value($claim,'"message":"','"');
if(strpos($claim, 'Promo kamu sudah bisa dipakai')){
$data = '{/go-promotions/v1/promotions/enrolment", $token, $data);{"promo_code":"COBAGOFOOD2206"}';
$claim = request($claim1,'"message":"','"');
message = fetch_value($claim,'"message":"','"');
if(strpos($claim, 'Promo kamu sudah bisa dipakai')){
$data = '{/go-promotions/v1/promotions/enrolment", $token, $data);{"promo_code":"PESANGOFOOD2206"}';
$claim = request($claim1,'"message":"','"');
message = fetch_value($claim,'"message":"','"');
if(strpos($claim, 'Promo kamu sudah bisa dipakai')){
reff:
